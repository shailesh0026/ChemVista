...
struct VisualizeAtom: Identifiable, Equatable {
    let id: UUID
...
struct VisualizeBond: Identifiable, Equatable {
    let id: UUID
    let fromAtomId: UUID
    let toAtomId: UUID
    var type: BondType
    ...
    enum BondType: Int, CaseIterable {
        ...
        var description: String {
            switch self {
            case .single: return "Single VisualizeBond"
            case .double: return "Double VisualizeBond"
            case .triple: return "Triple VisualizeBond"
            }
        }
    }
}
...
struct VisualizeMolecule: Identifiable, Equatable {
    let id: UUID
    var atoms: [VisualizeAtom]
    var bonds: [VisualizeBond]
    var highlightedAtoms: Set<UUID> = []

    init(id: UUID = UUID(), atoms: [VisualizeAtom] = [], bonds: [VisualizeBond] = [], highlightedAtoms: Set<UUID> = []) {
        self.id = id
        self.atoms = atoms
        self.bonds = bonds
    ...
    func atom(with id: UUID) -> VisualizeAtom? {
        atoms.first { $0.id == id }
    }
    ...
    // Returns the total bond order (valency used) for an atom.
    // Single = 1, Double = 2, Triple = 3.
    func valency(for atomId: UUID) -> Int {
        bonds
            .filter { $0.fromAtomId == atomId || $0.toAtomId == atomId }
            .reduce(0) { $0 + $1.type.rawValue }
    }
}


// --- VisualizeMolecule+Formula ---
//
//  VisualizeMolecule+Formula.swift
//  ChemVista
//
//  Created by Shailesh on 20/02/26.
//
...
// --- VisualizeMoleculeBuilder ---
//
//  VisualizeMoleculeBuilder.swift
//  ChemVista
//
//  Created by Shailesh on 20/02/26.
//
...
@Observable
class VisualizeMoleculeBuilder {
    var molecule: VisualizeMolecule

    init(molecule: VisualizeMolecule = VisualizeMolecule()) {
        self.molecule = molecule
    }

    // VisualizeAtom Operations

    @discardableResult
    func addAtom(element: VisualizeAtom.Element, position: SIMD3<Float>) -> VisualizeAtom {
        let atom = VisualizeAtom(position: position, element: element)
        molecule.atoms.append(atom)
        return atom
    }

    func removeAtom(_ atom: VisualizeAtom) {
        // Batch both removals into a single molecule update to avoid mid-mutation publishes.
        var mol = molecule
        mol.bonds.removeAll { $0.fromAtomId == atom.id || $0.toAtomId == atom.id }
        mol.atoms.removeAll { $0.id == atom.id }
        molecule = mol
    }

    // VisualizeBond Operations

    func bondExists(from: UUID, to: UUID) -> VisualizeBond? {
    ...
    @discardableResult
    func addBond(from fromAtom: VisualizeAtom, to toAtom: VisualizeAtom, type: VisualizeBond.BondType = .single) -> Bool {
        ...
        var mol = molecule
        mol.bonds.append(VisualizeBond(from: fromAtom.id, to: toAtom.id, type: type))
        molecule = mol
        return true
    }

    func removeBond(_ bond: VisualizeBond) {
        molecule.bonds.removeAll { $0.id == bond.id }
    }

    // VisualizeMolecule-level Operations

    func clear() {
        molecule = VisualizeMolecule()
    }

    func setMolecule(_ molecule: VisualizeMolecule) {
        self.molecule = molecule
    }

    // Helpers

    private func currentValency(for atom: VisualizeAtom) -> Int {
        molecule.bonds
...
// --- VisualizeMolecule3DView ---
//
//  VisualizeMolecule3DView.swift
//  ChemVista
//
//  Created by Shailesh on 20/02/26.
//
...
struct VisualizeMolecule3DView: UIViewRepresentable {
    @Binding var molecule: VisualizeMolecule
    var cameraDistance: Float = 25
    
    func makeUIView(context: Context) -> SCNView {
        let view = SCNView()
        let scene = VisualizeMoleculeScene(cameraDistance: cameraDistance)
        view.scene = scene
    ...
    func updateUIView(_ uiView: SCNView, context: Context) {
        if let scene = uiView.scene as? VisualizeMoleculeScene {
...
// --- VisualizeMoleculeScene ---
//
//  VisualizeMoleculeScene.swift
//  ChemVista
//
//  Created by Shailesh on 20/02/26.
//
...
class VisualizeMoleculeScene: SCNScene {
    var molecule: VisualizeMolecule?
    ...
    func update(with newMolecule: VisualizeMolecule) {
        ...
        for bond in newMolecule.bonds {
            ...
            } else if let fromAtom = newMolecule.atom(with: bond.fromAtomId),
                      let toAtom = newMolecule.atom(with: bond.toAtomId),
                      let existingBondNode = bondNodes[bond.id] {
                existingBondNode.removeFromParentNode()
                let newNode = createBondNode(from: fromAtom, to: toAtom, type: bond.type, centerOffset: center)
                newNode.opacity = existingBondNode.opacity
    ...
    func createAtomNode(for atom: VisualizeAtom, centerOffset: SIMD3<Float> = .zero) -> SCNNode {
    ...
    func createBondNode(from item1: VisualizeAtom, to item2: VisualizeAtom, type: VisualizeBond.BondType, centerOffset: SIMD3<Float> = .zero) -> SCNNode {
    ...
    func setHighlight(_ highlighted: Bool, for node: SCNNode, element: VisualizeAtom.Element) {
...
// --- VisualizeARMoleculeView ---
//
//  VisualizeARMoleculeView.swift
//  ChemVista
//
//  Created by Shailesh on 20/02/26.
//
...
@Observable
class VisualizeMoleculeFinderState {
...
struct VisualizeARMoleculeView: UIViewRepresentable {
    @Binding var molecule: VisualizeMolecule
    var finderState: VisualizeMoleculeFinderState
    ...
    class Coordinator: NSObject, ARSCNViewDelegate {
        ...
        private let finderState: VisualizeMoleculeFinderState

        init(finderState: VisualizeMoleculeFinderState) {
        ...
        func rebuildMolecule(_ molecule: VisualizeMolecule, in container: SCNNode) {
...
// VisualizeGlassCircleButton
struct VisualizeGlassCircleButton: View {
...
// VisualizeGlassPill
struct VisualizeGlassPill<Content: View>: View {
    let content: Content
...
// VisualizeMolecule Finder Overlay
struct VisualizeMoleculeFinderOverlay: View {
    var state: VisualizeMoleculeFinderState
...
// VisualizeBondSelectorView
struct VisualizeBondSelectorView: View {
    @Bindable var viewModel: VisualizeViewModel

    var body: some View {
        HStack(spacing: 0) {
            ForEach(VisualizeBond.BondType.allCases, id: \.rawValue) { bondType in
                Button {
                    ...
                    HStack(spacing: 6) {
                        ...
                        Text(bondLabel(for: bondType))
                            .font(.system(size: 13, weight: viewModel.selectedBondType == bondType ? .bold : .medium))
                    }
                    .foregroundColor(viewModel.selectedBondType == bondType ? .white : .white.opacity(0.45))
                    ...
                    .background(
                        viewModel.selectedBondType == bondType
                            ? AnyView(
                                Capsule()
                                    .fill(bondColor(for: bondType).opacity(0.25))
                                    .overlay(
                                        Capsule()
                                            .stroke(bondColor(for: bondType).opacity(0.5), lineWidth: 0.5)
                                    )
                            )
                            : AnyView(Color.clear)
                    )
                ...
                .accessibilityLabel("\(bondLabel(for: bondType)) bond")
                .accessibilityAddTraits(viewModel.selectedBondType == bondType ? .isSelected : [])
            }
        }
        .padding(4)
        .background(
    ...
    private func bondLabel(for type: VisualizeBond.BondType) -> String {
        switch type {
    ...
    private func bondColor(for type: VisualizeBond.BondType) -> Color {
        switch type {
    ...
    @ViewBuilder
    private func bondIcon(for type: VisualizeBond.BondType) -> some View {
        let symbolType: BondSymbolView.BondType = {
            switch type {
            case .single: return .single
            case .double: return .double
            case .triple: return .triple
        ...
        BondSymbolView(
            type: symbolType,
            color: viewModel.selectedBondType == type ? Color.white : Color.white.opacity(0.4)
        )
    }
}

struct VisualizeARVisualizerSheet: View {
    @Bindable var viewModel: VisualizeViewModel
    @Environment(\.dismiss) private var dismiss
    @State private var finderState = VisualizeMoleculeFinderState()
    @State private var triggerScreenshot = false
    @State private var showPhotoPermissionAlert = false
    @State private var showSaveSuccessOverlay = false
    private let permissionManager = PermissionManager.shared

    var body: some View {
        NavigationStack {
            ZStack {
                VisualizeARMoleculeView(molecule: $viewModel.molecule, finderState: finderState, triggerScreenshot: $triggerScreenshot)
                    .edgesIgnoringSafeArea(.all)

                // Directional finder arrows — auto-hide when molecule is on screen.
                VisualizeMoleculeFinderOverlay(state: finderState)

                // Molecule info pill directly below the navigation bar
                VStack {
                    VisualizeGlassPill {
                        if viewModel.molecule.atoms.isEmpty {
                            Text("First add Carbon, then Hydrogen")
                                .font(.system(size: 14, weight: .medium))
                                .foregroundColor(.white.opacity(0.85))
                        } else {
                            HStack(spacing: 8) {
                                Text(viewModel.moleculeName)
                ...
                VStack {
                    ...
                    VStack(spacing: 16) {
                        ...
                        HStack {
                            ...
                            AtomStepper(
                                symbol: "H", color: .white, textColor: .black,
                                onAdd:    { withAnimation { viewModel.addHydrogen() } },
                            ...
                            AtomStepper(
                                symbol: "C", color: Color(red: 0.0, green: 0.8, blue: 0.85), textColor: .white,
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    HStack(spacing: 16) {
                        Button { withAnimation { viewModel.clear() } } label: {
                            Image(systemName: "trash")
                                .font(.system(size: 20, weight: .medium))
                                .foregroundColor(.red)
                        }
                        .buttonStyle(ToolbarIconStyle())
                        .opacity(viewModel.molecule.atoms.isEmpty ? 0.35 : 1.0)
                        .disabled(viewModel.molecule.atoms.isEmpty)
                        .accessibilityLabel("Clear molecule")
                        .accessibilityHint("Removes all atoms")
                    }
                }
            }
        }
...
// VisualizeARViewOnlySheet
struct VisualizeARViewOnlySheet: View {
    @Binding var molecule: VisualizeMolecule
    let title: String
    @Environment(\.dismiss) private var dismiss
    @State private var finderState = VisualizeMoleculeFinderState()
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                VisualizeARMoleculeView(molecule: $molecule, finderState: finderState, triggerScreenshot: .constant(false))
                    .edgesIgnoringSafeArea(.all)
...
// --- VisualizeIUPACNamingLogic ---
//
//  VisualizeIUPACNamingLogic.swift
//  ChemVista
//
//  Created by Shailesh on 20/02/26.
//
...
struct VisualizeIUPACNamingLogic {

    static func generateName(for molecule: VisualizeMolecule) -> String {
        ...
        for i in 0..<chainLength - 1 {
            ...
            }), bond.type.rawValue > highestBondOrder {
                highestBondOrder = bond.type.rawValue
            }
        }

        if highestBondOrder == 2 { suffix = "ene" }
        if highestBondOrder == 3 { suffix = "yne" }
    ...
    static func calculateTheoreticalHydrogens(for molecule: VisualizeMolecule) -> Int {
        let carbons = molecule.atoms.filter { $0.element == .carbon }
    ...
    static func findLongestCarbonChain(in molecule: VisualizeMolecule) -> [VisualizeAtom] {
        let carbons = molecule.atoms.filter { $0.element == .carbon }
    ...
    private static func dfsLongestChain(current: VisualizeAtom, visited: Set<UUID>, molecule: VisualizeMolecule) -> [VisualizeAtom] {
        ...
        let best = neighbors.map { neighbor -> [VisualizeAtom] in
            var seen = visited
